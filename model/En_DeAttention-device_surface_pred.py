...
corpus = '../../dataset/trainingData/Dec11_sur_sep/'
...
class Voc:
    def __init__(self, name):
        ...
        self.index2word = {PAD_token: "PAD", SOS_token: "SOS", EOS_token: "EOS", UNK_token: "<UNKNOWN>",
                           NUMBER_token: "<NUMBER>",YEAR_token: "<YEAR>", SEP_token: 'surfaceformseparation'}
...
class vocab():
    def __init__(self, preTrainemb_path,preTrainemb_file,preTrainembVocabSize,embedding_size, 
        ...
        self.index2word = {}
        self.specialToken = ["PAD","SOS","EOS","<UNKNOWN>","<NUMBER>","<YEAR>", "surfaceformseparation"]
        self.name = name
        ...
        if createEmbeddingMatrixFlag==True:
            self.vectors = bcolz.carray(np.random.normal(scale=0.6, size=(self.index,self.embedding_size)),
                                    rootdir=self.preTrainemb_path+self.name+'.dat', mode='w')
    ...
    def saveFiles(self):
        ...
        #vectors.flush()
        pickle.dump(self.embedding_matrix,open(self.preTrainemb_path+self.name+'_embMat.pkl', 'wb') )
        pickle.dump(self.words, open(self.preTrainemb_path+self.name+'_words.pkl', 'wb'))
        pickle.dump(self.word2index, open(self.preTrainemb_path+self.name+'_w2i.pkl', 'wb'))
        pickle.dump(self.index2word, open(self.preTrainemb_path+self.name+'_i2w.pkl', 'wb'))
    
    def readFiles(self):
        self.vectors = bcolz.open(self.preTrainemb_path+self.name+'.dat')[:]
        self.words = pickle.load(open(self.preTrainemb_path+self.name+'_words.pkl', 'rb'))
        self.word2index = pickle.load(open(self.preTrainemb_path+self.name+'_w2i.pkl', 'rb'))
        self.index2word = pickle.load(open(self.preTrainemb_path+self.name+'_i2w.pkl', 'rb'))
        self.embedding_matrix = pickle.load(open(self.preTrainemb_path+self.name+'_embMat.pkl', 'rb') )
...
def batch2TrainData(voc, pair_batch):
    ...
    for pair in pair_batch:
        #print (pair[1].encode('utf-8'), pair[3].encode('utf-8'))
        input_batch.append(pair[1])
        output_batch.append(pair[3]) #need to be change according to dataFile
...
def evaluateInput(encoder, decoder, searcher, voc, pair):
        ...
        q_values = pair[4]
        df = pd.DataFrame()
        d = {'OrgSeq1':pair[1].strip(),'TarSeq2':tar_sent, 'PreSeq2':pred_sent, 'WikiUri2':q_values, 'WikiSeq2':wiki_sent}
        df = pd.DataFrame(data=d, index=[0])
        #df_total = df_total.append(df,ignore_index=True)
        with open(save_dir+'/'+str(epoch)+'predicted.csv', 'a', encoding="utf-8") as f:
...
for epoch in range(starting_epoch,epochs):
    ...
    if (epoch%7==0):
        ...
        accuracies_train = [evaluateInput(encoder, decoder, searcher,voc, pair) for pair in trainPairs]
    ...
    # Set dropout layers to eval mode
        ...
        encoder.train()
        decoder.train()
    with open('epoch_loss_Dec11_sur_sep.npy','ab') as f:   #NTBC
